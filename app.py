import psutil
import json, time, cv2, torch, argparse, flask, logging, socket, os, numpy
...
parser.add_argument(
    "--weights",
    type=str,
    default=os.path.join(os.path.dirname(__file__), "weights/v7.pt"),
)
        ...
        while not exit_flag.is_set():
            ...
            for xyxy_array, xywh_array in zip(xyxy, xywh):
                for xywh, xyxy in zip(
                    ...
                    cornerRect(
                        original_frame,
                        [int(x) for x in (xyxy[0], xyxy[1], xywh[2], xywh[3])],
                        colorC=COLOR,
                    )
